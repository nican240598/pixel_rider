              ...
              >
                <div>
                  <div className="relative h-80 w-full overflow-hidden bg-black">
                    ...
                    />
                    <div className="absolute top-3 left-3 bg-black/75 backdrop-blur-md pl-1.5 pr-3 py-1 rounded-full text-[11px] font-bold text-amber-400 flex items-center gap-1.5 shadow-md border border-white/10">
                      <UserAvatar
                        username={b.owner}
                        allUsers={allUsers}
                      ...
                      />
                      <span>{b.owner}</span>
                      <UserRoleBadge username={b.owner} allUsers={allUsers} currentUser={currentUser} size="xs" />
                    </div>
                    {b.images.length > 1 && (
                ...
                previewBike.comments?.map((c, i) => (
                  <div key={i} className="bg-slate-900/80 p-2.5 rounded-lg border border-slate-800 text-xs">
                    <div className="flex items-center gap-2 mb-1 flex-wrap">
                      ...
                      />
                      <strong className="text-purple-400 font-bold">{c.author}</strong>
                      <UserRoleBadge username={c.author} allUsers={allUsers} currentUser={currentUser} size="xs" />
                    </div>
                    <span className="text-slate-300 whitespace-pre-wrap">{c.text}</span>
                  </div>
                ))
              )}
            </div>
