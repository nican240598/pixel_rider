...
/**
 * Determine user rank title & badge styling based on total driven KM
 */
export function getUserRankBadge(totalKm: number): UserRankBadge {
  if (totalKm >= 5000) {
    return {
      title: 'Asphalt-Legende',
      icon: '🔥',
      badgeClass: 'bg-purple-950/80 text-purple-300 border-purple-500/60 shadow-[0_0_12px_rgba(168,85,247,0.4)]',
    };
  }
  if (totalKm >= 1000) {
    return {
      title: 'Touring-Fan',
      icon: '🏍️',
      badgeClass: 'bg-amber-950/80 text-amber-300 border-amber-500/60 shadow-[0_0_10px_rgba(245,158,11,0.3)]',
    };
  }
  return {
    title: 'Schönwetterfahrer',
    icon: '🌤️',
